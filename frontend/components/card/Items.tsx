...
const Items = ({lable,url,filename}:Props) => {
  return (
    <div className={` ${lable==='Delete' ? 'hover:bg-red-700': `${lable==='Download' ? 'hover:bg-blue-500': 'hover:bg-purple-700 '}`} text-[1.1rem] gap-5 flex justify-start items-center font-light px-3 rounded-2xl h-8`}>
      <div className={`w-5 peer peer-hover:animate-bounce hover:animate-bounce`}>
        {lable==='Edit' && <Edit/>}
        {lable==='Download' && <Downloads/>}
        {lable==='Delete' && <Delete/>}
      </div>
      <div className='peer hover:animate-bounce peer-hover:animate-bounce'>{lable==='Download' ? <Download url={`${url}`} name={filename}/> : <p>{lable}</p>}</div>
    </div>
  )
}
